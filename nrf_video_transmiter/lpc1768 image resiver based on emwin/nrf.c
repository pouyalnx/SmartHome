...
void nrf_set_address(unsigned char *address,unsigned char pipe){
	switch(pipe){
		case 0:
			if(address[0]!=nrf_prof.rx_p0_address[0] || address[1]!=nrf_prof.rx_p0_address[1] || address[2]!=nrf_prof.rx_p0_address[2] || address[3]!=nrf_prof.rx_p0_address[3] || address[4]!=nrf_prof.rx_p0_address[4]){
			nrf_write_command(0x20|0xa,address,5);
		  memcpy(nrf_prof.rx_p0_address,address,5);
		}
		break;
		case 1:
			if(address[0]!=nrf_prof.rx_p1_address[0] || address[1]!=nrf_prof.rx_p1_address[1] || address[2]!=nrf_prof.rx_p1_address[2] || address[3]!=nrf_prof.rx_p1_address[3] || address[4]!=nrf_prof.rx_p1_address[4]){
   		nrf_write_command(0x20|0xb,address,5);
		  memcpy(nrf_prof.rx_p1_address,address,5);			
		  }	
		break;
		case 100:
			if(address[0]!=nrf_prof.tx_address[0] || address[1]!=nrf_prof.tx_address[1] || address[2]!=nrf_prof.tx_address[2] || address[3]!=nrf_prof.tx_address[3] || address[4]!=nrf_prof.tx_address[4]){

...
unsigned char tx_address[5]={0xab,0xac,0xfa,0xfd,0xaa};
unsigned char tx_channel=8;

unsigned char search_address[5]={0xf,0xa,0xf,0xa,25};
unsigned char sof_pocket[32];
unsigned char transmitter_name[16]="video link0     ";
...
//	stdout_cmdmode(1);
  osDelay(100);
  gpv0=cam_init();
	gpv0=cam_image((unsigned char  *)image);
	gpv1=0;
	while(1){
  gpv1+=100;
//air handshaking

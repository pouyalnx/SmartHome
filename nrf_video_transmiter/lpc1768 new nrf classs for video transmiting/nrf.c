...
void nrf_enable_rf(int stat){
	if(stat){
    LPC_GPIO0->FIOSET=512;		
		nrf_prof.rf_stat=1;
	}
	else{
    LPC_GPIO0->FIOCLR=512;		
		nrf_prof.rf_stat=0;		
	}
}
...
int nrf_irq_stat(void){
	return LPC_GPIOINT->IO0IntStatF&256;
}

void nrf_irq_clr_stat(void){
	nrf_write_stat(0x70);
	LPC_GPIOINT->IO0IntClr=256;
}
...
void nrf_init(void){
  ...
  nrf_prof.rf_stat=0;  
	...
	LPC_GPIO0->FIODIR&=~(256);
	LPC_GPIO0->FIODIR|=512;
//	LPC_PINCON->PINSEL0&=~0XF;
 // LPC_PINCON->PINMODE0&=~0XF;
  LPC_GPIO0->FIOCLR=512;
  LPC_GPIOINT->IO0IntClr=256;
	LPC_GPIOINT->IO0IntEnF=256;
